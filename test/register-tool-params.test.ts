...
describe("registerArmoryTool — parameter interpolation", () => {
  ...
  describe("flag parameters — flag+value", () => {
    ...
    it("preserves whitespace inside shell-escaped values", () => {
      const result = interpolateCommand("echo {{--message msg?}} {{text}}", {
        msg: "hello  world",
        text: "line1\n  line2",
      });
      expect(result).toBe("echo --message 'hello  world' 'line1\n  line2'");
    });

    it("preserves tabs and newlines in unquoted regions", () => {
      const result = interpolateCommand("cmd1\ncmd2 {{--verbose?}}", { verbose: true });
      expect(result).toBe("cmd1\ncmd2 --verbose");
    });

    it("preserves tabs in unquoted regions", () => {
      const result = interpolateCommand("cmd1\t{{--verbose?}}", { verbose: true });
      expect(result).toBe("cmd1\t--verbose");
    });

    it("does not treat prototype properties as provided params", () => {
      expect(() => interpolateCommand("run {{--toString}}", {})).toThrow("Missing required parameter: toString");
    });
  });
  ...
  describe("flag parameters — conflict detection", () => {
    it("throws when boolean flag and flag+value share the same param name", () => {
      // {{--verbose}} derives param name "verbose"; {{--verbose verbose}} also uses "verbose"
      expect(() => parsePlaceholders("cmd {{--verbose}} {{--verbose verbose}}")).toThrow(
        "Conflicting modifiers for placeholder: verbose",
      );
    });

    it("throws when flag+value conflicts with existing boolean flag of same name", () => {
      // {{--verbose verbose}} creates param "verbose" as flag+value;
      // {{--verbose}} creates param "verbose" as boolean — conflict
      expect(() => parsePlaceholders("cmd {{--verbose verbose}} {{--verbose}}")).toThrow(
        "Conflicting modifiers for placeholder: verbose",
      );
    });

    it("allows different param names even with same-looking flag", () => {
      // {{--msg message}} creates param "message"; {{--verbose}} creates param "verbose" — no conflict
      expect(() => parsePlaceholders("cmd {{--msg message}} {{--verbose}}")).not.toThrow();
    });

    it("throws when flag placeholder conflicts with regular placeholder of same name", () => {
      // {{--verbose}} creates boolean param "verbose"; {{verbose}} is a regular param "verbose" — conflict
      expect(() => parsePlaceholders("cmd {{--verbose}} {{verbose}}")).toThrow(
        "Conflicting modifiers for placeholder: verbose",
      );
    });

    it("throws when flag+value placeholder conflicts with regular placeholder of same name", () => {
      // {{--message msg}} creates param "msg"; {{msg}} is a regular param "msg" — conflict
      expect(() => parsePlaceholders("cmd {{--message msg}} {{msg}}")).toThrow(
        "Conflicting modifiers for placeholder: msg",
      );
    });

    it("allows same flag placeholder repeated identically", () => {
      expect(() => parsePlaceholders("cmd {{--verbose?}} {{--verbose?}}")).not.toThrow();
      expect(() => parsePlaceholders("cmd {{--message msg?}} {{--message msg?}}")).not.toThrow();
    });
  });
});
